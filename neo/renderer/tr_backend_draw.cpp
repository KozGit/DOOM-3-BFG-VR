...
void RB_DrawDistortionMesh( int eye, bool timewarp, int ocuframe, ovrPosef thePose )
{
	...
	if ( backEnd.glState.vertexLayout != layout ) // vertex layout is not correct for this eye
	{
		...
		glBindBuffer( GL_ARRAY_BUFFER, vr->hmdEye[eye].vbo.vertBuffHandle );
		glBindBuffer( GL_ELEMENT_ARRAY_BUFFER, vr->hmdEye[eye].vbo.indexBuffHandle );
		...
		backEnd.glState.currentIndexBuffer = vr->hmdEye[eye].vbo.indexBuffHandle;
		backEnd.glState.currentVertexBuffer = vr->hmdEye[eye].vbo.vertBuffHandle;
	...
	glDrawElements( GL_TRIANGLES, vr->hmdEye[eye].vbo.numIndexes, GL_UNSIGNED_SHORT, (void*)0 );
