...
#include "framework\Common_local.h" // koz
#include "AutoRender.h" // koz
...
int idAutoRender::Run() {
	...
	//while ( !IsTerminating() ) {
		...
		//session->Pump();
		//common->Printf("Autorender thread running\n");
		//Sys_Sleep(10);
...
void idAutoRender::StartBackgroundAutoSwaps( autoRenderIconType_t iconType ) {
	...
	//common->Printf("Starting Background AutoSwaps\n");
	...
	StartThread("BackgroundAutoSwaps", CORE_ANY, THREAD_LOWEST, AUTO_RENDER_STACK_SIZE );
}
...
void idAutoRender::EndBackgroundAutoSwaps() {
	//common->Printf("End Background AutoSwaps\n");
...
void idAutoRender::RenderFrame() {
	static const int MAX_FRAMES = 60;
