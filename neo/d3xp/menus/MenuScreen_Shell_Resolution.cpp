...
void idMenuScreen_Shell_Resolution::ShowScreen( const mainMenuTransition_t transitionType )
{
	...
	for( int displayNum = 0 ; displayNum < displays.Num(); displayNum++ )
	{
		...
		for( int i = 0; i < modeList.Num(); i++ )
		{
			...
			/*	Koz begin : list monitor names to more easily identify a Rift in the monitor list,
				as display numbers don't match windows numbering.  Retrieving the actual monitor
				model name is cumbersome, and the potential is there to fail in some windows instances,
				so it can be disabled via cvar vr_listMonitorName if problematic.
				Obviously for windows only.
			*/
