...
void idProgram::Save( idSaveGame* savefile ) const
{
	...
	//common->Printf("idProgram::Save() count:%d @%d\n", fileList.Num() - currentFileNum, savefile->file->Tell()); //Npi debug
	...
	//common->Printf("idProgram::Save() overrided variables:%d  @%d\n", variableDefaults.Num(), savefile->file->Tell()); //Npi debug
	
	int start = savefile->file->Tell(); //Npi debug
	int writeCount = 0;
	...
	savefile->WriteInt( -1 );
	//common->Printf("idProgram::Save() overrided variables:%d/%d, %d bytes\n", writeCount, variableDefaults.Num(), savefile->file->Tell()- start); //Npi debug

	//common->Printf("idProgram::Save() other variables:%d @%d\n", numVariables-variableDefaults.Num(), savefile->file->Tell()); //Npi debug
...
================
*/
bool idProgram::Restore( idRestoreGame* savefile, int &skill_level, idStr &first_decl_string, idStr &second_decl_string)
{
	int i, num, index;
	bool result = true;
	idStr scriptname;
	...
	while( index >= 0 ) //detect the Mark the end of the diff with default variables with -1
	{
		savefile->ReadByte( variables[index] );
		savefile->ReadInt( index );
		if(minVariableDefaultsNum < index + 1 )
			minVariableDefaultsNum = index + 1;
	}
	
	savefile->ReadInt( num ); //numVariables

	//common->Printf("idProgram::Restore() Read variables after defaults=%d, num=%d, %d\n", minVariableDefaultsNum, num, savefile->file->Tell() - 4); //Carl debug
	...
	// Npi : then another 4-byte integer pointing to a valid string in the strings file.
	...
	// Npi : And if an unlucky random offset match a acceptable string
	...
	//
	// Npi : May use a version number in savegame instead (v022 is the good solution), and switch between version's restore methods.
	// With "Lost Mission"'s levels we found tese conditions by luck before the true maxCount, and all next reads are dead..
	// To fix that quickly, just look for a second string pointer : before 1% chance to fail, now 1%*1%=0.01% chance to fail ... it's better, but... :)
	// before that I try to look for checksum, but old saves have a different checksum
	...
	int saved_checksum, skill = -1, string1Pointer, string1Length, string2Pointer, string2Length;
	idStr actualString1 = "";
	idStr actualString2 = "";
	// a buffer large enough to hold the four 4-byte values after our bytes
	byte circleBuffer[16];
	...
	while (i < maxCount)
	{
		...
		saved_checksum = circleBuffer[circleBufferIndex] | (circleBuffer[(circleBufferIndex + 1) % 16] << 8) | (circleBuffer[(circleBufferIndex + 2) % 16] << 16) | (circleBuffer[(circleBufferIndex + 3) % 16] << 24);
		saved_checksum = BigLong(saved_checksum);
		skill = circleBuffer[(circleBufferIndex + 4) % 16] | (circleBuffer[(circleBufferIndex + 5) % 16] << 8) | (circleBuffer[(circleBufferIndex + 6) % 16] << 16) | (circleBuffer[(circleBufferIndex + 7) % 16] << 24);
		skill = BigLong(skill);
		if (skill >= 0 && skill <= 3) // 1 = marine, 3 = nightmare
		{
			string1Pointer = circleBuffer[(circleBufferIndex + 8) % 16] | (circleBuffer[(circleBufferIndex + 9) % 16] << 8) | (circleBuffer[(circleBufferIndex + 10) % 16] << 16) | (circleBuffer[(circleBufferIndex + 11) % 16] << 24);
			string1Pointer = BigLong(string1Pointer);
			if (string1Pointer > 16 && string1Pointer <= savefile->stringFile->Length() - 4)
			{
				string2Pointer = circleBuffer[(circleBufferIndex + 12) % 16] | (circleBuffer[(circleBufferIndex + 13) % 16] << 8) | (circleBuffer[(circleBufferIndex + 14) % 16] << 16) | (circleBuffer[(circleBufferIndex + 15) % 16] << 24);
				string2Pointer = BigLong(string2Pointer);
				if (string2Pointer > 16 && string2Pointer <= savefile->stringFile->Length() - 4)
				{
					savefile->stringFile->Seek(string1Pointer, FS_SEEK_SET);
					savefile->stringFile->ReadInt(string1Length);
					if (string1Length >= 2 && string1Length <= 255) //Carl: I'm ASSUMING the first table name won't be longer than 255 chars
					{
						savefile->stringFile->Seek(string2Pointer, FS_SEEK_SET);
						savefile->stringFile->ReadInt(string2Length);
						if (string2Length >= 2 && string2Length <= 255) //Npi: I'm ASSUMING the second table name won't be longer than 255 chars
						{
							actualString1.Fill(' ', string1Length);
							savefile->stringFile->Seek(string1Pointer, FS_SEEK_SET);
							savefile->stringFile->ReadInt(string1Length);
							savefile->stringFile->Read(&actualString1[0], string1Length);
							
							actualString2.Fill(' ', string2Length);
							savefile->stringFile->Seek(string2Pointer, FS_SEEK_SET);
							savefile->stringFile->ReadInt(string2Length);
							savefile->stringFile->Read(&actualString2[0], string2Length);
		...
		// increment index
		circleBufferIndex = (circleBufferIndex + 1) % 16;
		i++;
	}
	// go through temp
	maxCount = i;
	...
	if (!malloc_failed)
		Mem_Free16(temp);
	// set our values
	if (skill == -1) //if we just read the good num of data
	{
		saved_checksum = circleBuffer[circleBufferIndex] | (circleBuffer[(circleBufferIndex + 1) % 16] << 8) | (circleBuffer[(circleBufferIndex + 2) % 16] << 16) | (circleBuffer[(circleBufferIndex + 3) % 16] << 24);
		saved_checksum = BigLong(saved_checksum);
		skill = circleBuffer[(circleBufferIndex + 4) % 16] | (circleBuffer[(circleBufferIndex + 5) % 16] << 8) | (circleBuffer[(circleBufferIndex + 6) % 16] << 16) | (circleBuffer[(circleBufferIndex + 7) % 16] << 24);
	...
	skill_level = skill;
	first_decl_string = actualString1;
	second_decl_string = actualString2;

	//common->Printf("idProgram::Restore(), num=%d, %d bytes, %d\n", maxCount, savefile->file->Tell() - start - 16, start); //Carl debug
	int checksum = CalculateChecksum();
	//common->Printf("idProgram::Restore() saved_checksum:%d computed:%d, @%d\n", saved_checksum, checksum, savefile->file->Tell()); //Npi debug
